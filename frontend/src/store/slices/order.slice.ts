...
const orderSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(
            getAllOrders.fulfilled, (state, action: PayloadAction<IOrderFromDbInterface[] | undefined>) => {
                if (action.payload)
        ...
        builder.addCase(saveOrderInDb.pending, (state) => {
            state.loading = true
        })
    },
});
...
export {orderReducer};
export const {setOrder, clearState, inc, dec, removeItem, setLoaderFalse} =
    orderSlice.actions;
